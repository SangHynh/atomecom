...
authRouter.post(
  ...
  asyncHandler(authControllerImpl.refresh),
);
authRouter.post(
  '/auth/logout',
  validate(TokenRequestSchema),
  asyncHandler(authControllerImpl.logout),
);
...
authRouter.post(
  ...
  asyncHandler(authControllerImpl.resendVerification),
);
authRouter.post(
  '/auth/forgot-password',
  validate(EmailOnlyRequestSchema),
  asyncHandler(authControllerImpl.forgotPassword),
);
authRouter.post(
  '/auth/reset-password',
  validate(ResetPasswordRequestSchema),
  asyncHandler(authControllerImpl.resetPassword),
);

// OAuth 2.0
authRouter.post(
  '/auth/social/:provider',
  validate(SocialLoginRequestSchema),
  asyncHandler(authControllerImpl.socialLogin),
);
