...
describe('EmailListener', () => {
  ...
  it('should handle USER_CREATED event', async () => {
    const handler = mockEventBus.on.mock.calls.find(
      (call) => call[0] === DomainEvents.USER_CREATED,
    )![1];

    await handler({ userId: 'u1', email: 'test@ex.com' });

    expect(mockMailTokenService.createMailToken).toHaveBeenCalledWith(
      'u1',
      'test@ex.com',
      'EMAIL_VERIFICATION',
    );
  ...
  it('should handle PASSWORD_RESET_REQUESTED event', async () => {
    const handler = mockEventBus.on.mock.calls.find(
      (call) => call[0] === DomainEvents.PASSWORD_RESET_REQUESTED,
    )![1];
  ...
  it('should handle VERIFICATION_EMAIL_REQUESTED event', async () => {
    const handler = mockEventBus.on.mock.calls.find(
      (call) => call[0] === DomainEvents.VERIFICATION_EMAIL_REQUESTED,
    )![1];
  ...
  it('should log error if email sending fails', async () => {
    ...
    await expect(
      handler({ userId: 'u1', email: 'test@ex.com' }),
