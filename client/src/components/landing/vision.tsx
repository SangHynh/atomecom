...
export function Vision() {
  ...
  return (
    <section id="vision" className="relative py-24 sm:py-32 bg-muted/40 border-y border-border/40 scroll-mt-16">
      <div className="container mx-auto max-w-7xl px-6 lg:px-16">
        ...
        >
          <h3 className="text-xl sm:text-2xl font-bold uppercase tracking-[0.15em] text-foreground/80 mb-3">{t('landing.stats_title')}</h3>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            >
              <div className={`rounded-2xl sm:rounded-3xl bg-primary/5 p-4 sm:p-5 text-primary transition-all duration-700 group-hover:bg-primary/10 ${item.color} transform group-hover:scale-125 group-hover:rotate-[15deg] group-hover:shadow-lg group-hover:shadow-primary/5`}>
                {item.icon}
              </div>
              <div className="text-center space-y-3 sm:space-y-4 relative z-10">
                <h4 className="text-lg sm:text-xl font-bold tracking-tight group-hover:text-primary transition-colors duration-700">{item.title}</h4>
                <p className="text-[11px] sm:text-xs text-muted-foreground/80 leading-relaxed font-medium line-clamp-4 group-hover:text-muted-foreground transition-colors duration-700">{item.desc}</p>
              </div>
