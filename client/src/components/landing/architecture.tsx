...
import { Zap, MousePointer2, ShieldCheck, RefreshCcw, Database, Rocket, Layers, Code2 } from "lucide-react";
...
export function Architecture() {
  ...
  return (
    <section id="architecture" className="py-24 sm:py-32 overflow-hidden bg-muted/20 border-y border-border/40 scroll-mt-16">
        ...
        <div className="flex flex-col items-center gap-16 lg:gap-20 lg:flex-row">
          ...
          >
            <div className="space-y-4 sm:space-y-6 text-center lg:text-left">
              <h3 className="text-3xl sm:text-4xl lg:text-6xl font-bold tracking-tight text-primary font-heading leading-tight">{t('landing.arch_title')}</h3>
            ...
            <div className="grid gap-4 sm:gap-6 sm:grid-cols-2">
              {[
                { icon: <Zap className="h-5 w-5"/>, title: t('landing.domain_centric'), desc: t('landing.domain_centric_desc'), color: "group-hover:text-yellow-500" },
                { icon: <MousePointer2 className="h-5 w-5"/>, title: t('landing.independent_view'), desc: t('landing.independent_view_desc'), color: "group-hover:text-rose-500" },
                { icon: <ShieldCheck className="h-5 w-5"/>, title: t('landing.highly_testable'), desc: t('landing.highly_testable_desc'), color: "group-hover:text-emerald-500" },
                { icon: <RefreshCcw className="h-5 w-5"/>, title: t('landing.decoupled_arch'), desc: t('landing.decoupled_arch_desc'), color: "group-hover:text-indigo-500" },
                { icon: <Database className="h-5 w-5"/>, title: t('landing.flexible_tech'), desc: t('landing.flexible_tech_desc'), color: "group-hover:text-sky-500" },
                { icon: <Rocket className="h-5 w-5"/>, title: t('landing.future_proof'), desc: t('landing.future_proof_desc'), color: "group-hover:text-amber-500" }
              ].map((item, idx) => (
                <motion.div 
                  key={idx}
                  whileHover={{ x: 10 }}
                ...
                >
                  <div className="mt-1 rounded-xl sm:rounded-2xl bg-primary/10 p-2 transition-all duration-700 group-hover:bg-primary/20 group-hover:rotate-6">
                    <div className={`text-primary transition-colors duration-700 ${item.color}`}>{item.icon}</div>
                  </div>
                  <div>
                    <h4 className="font-bold text-base sm:text-lg text-foreground mb-1 group-hover:text-primary transition-colors duration-700">{item.title}</h4>
                    <p className="text-[10px] sm:text-xs text-muted-foreground/70 font-medium leading-normal">{item.desc}</p>
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="aspect-square rounded-[2rem] sm:rounded-[3rem] bg-gradient-to-tr from-primary/80 to-indigo-600/80 p-8 sm:p-12 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] relative overflow-hidden group cursor-default">
              ...
              <div className="relative h-full flex flex-col justify-center items-center text-white space-y-6 sm:space-y-8">
                ...
                <div className="text-center space-y-2">
                  <p className="text-xl sm:text-2xl font-black tracking-[0.2em] uppercase transition-all duration-700 group-hover:tracking-[0.3em]">{t('landing.ddd_title')}</p>
                  <p className="text-white/80 font-bold text-xs sm:text-sm bg-white/10 px-4 py-1 rounded-full backdrop-blur-md transition-all duration-700 group-hover:bg-white/20">{t('landing.ddd_desc')}</p>
                </div>
              </div>
            </div>
          </motion.div>
