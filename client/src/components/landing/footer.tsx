      ...
      <div className="container mx-auto max-w-7xl px-8 lg:px-14">
        <div className="flex flex-col items-center justify-between gap-12 md:flex-row">
          <div className="flex flex-col items-center gap-3 md:items-start min-w-[200px]">
            <Link href="/" className="flex items-center gap-2 cursor-pointer group" aria-label="Atomecom Home">
              ...
              <span className="text-2xl font-bold tracking-tight font-heading group-hover:text-primary transition-all duration-700">Atomecom</span>
            </Link>
            <p className="text-sm text-muted-foreground font-medium">{t('footer.rights')}</p>
          </div>
          
          <div className="flex gap-6 items-center justify-center">
            {[
              { icon: <Globe className="h-6 w-6" />, label: "Website", aria: "Visit our Website" },
              { icon: <Github className="h-6 w-6" />, label: "GitHub", aria: "Visit our GitHub" },
              { icon: <Linkedin className="h-6 w-6" />, label: "LinkedIn", aria: "Visit our LinkedIn" }
            ].map((social, idx) => (
              <Button 
                key={idx} 
                variant="ghost" 
