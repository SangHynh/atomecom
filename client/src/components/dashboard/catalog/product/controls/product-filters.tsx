    ...
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mb-8 sticky top-0 bg-background/80 backdrop-blur-md z-20 py-3 border-b border-border/10">
      ...
      <div className="flex items-center gap-3 overflow-x-auto no-scrollbar py-1">
        ...
        >
          ...
          <SelectContent className="rounded-sm border-[0.5px] border-border/40 shadow-none">
            <SelectItem
              value="all"
              className="text-[10px] uppercase font-bold tracking-wide rounded-md focus:bg-muted/50"
            >
              Tất cả sản phẩm
            </SelectItem>
            <SelectItem
              value={PRODUCT_STATUS.PUBLISHED}
              className="text-[10px] uppercase font-bold tracking-widest rounded-sm focus:bg-muted/50"
            >
              Đang hiển thị
            </SelectItem>
            <SelectItem
        ...
        <Button
          variant="outline"
          size="icon"
          className="h-11 w-11 rounded-md hover:bg-muted/10 transition-colors border border-border/10 group shrink-0"
          onClick={onRefresh}
        >
          <RefreshCw
            className={cn(
