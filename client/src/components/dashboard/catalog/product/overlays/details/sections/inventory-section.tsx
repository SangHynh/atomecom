      ...
      <div className="grid grid-cols-1 gap-2">
        {skus.length > 0 ? (
          skus.map((sku: Sku) => (
            <div
              key={sku.id}
              className="flex items-center justify-between p-4 rounded-[var(--radius)] border border-border/40 bg-muted/10 group hover:border-primary/20 transition-all"
            >
              <div className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-[var(--radius)] bg-background border border-border/20 overflow-hidden shrink-0">
                  {sku.images?.[0] ? (
                    <img
                      src={sku.images[0]}
                      alt={sku.name}
          ...
          <div className="p-8 text-center border border-dashed border-border/40 rounded-[var(--radius)]">
