...
const STATUS_DOT: Record<string, string> = {
  [PRODUCT_STATUS.PUBLISHED]: 'bg-emerald-500',
  [PRODUCT_STATUS.DRAFT]: 'bg-amber-500',
  [PRODUCT_STATUS.HIDDEN]: 'bg-zinc-400',
  [PRODUCT_STATUS.DISCONTINUED]: 'bg-red-500',
};
...
export function ProductCard({ product, onView }: ProductCardProps) {
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/5] overflow-hidden bg-muted/10 border border-border/40 rounded-sm">
        {product.thumbnail ? (
          <img
        ...
        <div className="absolute top-4 left-4 flex items-center gap-2">
          <div
            className={cn(
              ...
              STATUS_DOT[product.status] || 'bg-zinc-400',
            )}
          />
          <span className="text-[8px] font-bold uppercase tracking-wide text-white/90 bg-black/60 backdrop-blur-md px-2 py-0.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity">
            {product.status}
          </span>
        </div>
        ...
        <div className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/[0.02] transition-colors duration-500 flex items-center justify-center">
          <div className="opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
            <div className="h-12 w-12 rounded-sm bg-background/90 backdrop-blur-md flex items-center justify-center border border-border/40 shadow-none">
