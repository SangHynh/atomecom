...
export function BrandCard({ brand, index, onView }: BrandCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative rounded-[var(--radius)] border border-border/10 bg-background shadow-none hover:border-foreground/20 transition-all duration-500 flex flex-col h-full min-h-[280px] p-5 overflow-hidden"
    >
      ...
      <div className="absolute -top-1 -right-1 w-32 h-32 overflow-hidden z-20 pointer-events-none">
        <div
          className={cn(
            ...
            brand.status === PRODUCT_STATUS.HIDDEN && 'bg-muted-foreground/60 text-white',
      ...
      <div className="flex flex-col gap-5 flex-1">
        <div className="relative w-fit">
          ...
          <Avatar className="h-14 w-14 rounded-[var(--radius)] border border-border/20 shadow-none relative z-10 bg-white transition-transform duration-500 group-hover:scale-105">
            <AvatarImage
              src={brand.logo}
              alt={brand.name}
        ...
        <div className="pt-4 border-t border-border/10 flex items-center justify-between text-[9px] uppercase font-bold tracking-wide text-muted-foreground/30 mt-auto">
          <span className="flex items-center gap-1">
            <div className="w-1 h-1 rounded-[var(--radius)] bg-muted-foreground/30" />
            Join Date
          </span>
      ...
      <div className="mt-5">
        <Button
          className="w-full bg-foreground text-background hover:bg-foreground/90 rounded-[var(--radius)] h-10 text-[10px] font-bold uppercase tracking-wide transition-all duration-300 group/btn shadow-none flex items-center justify-between px-4"
