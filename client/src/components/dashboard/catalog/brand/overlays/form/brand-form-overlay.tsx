'use client';

import React from 'react';
import { BrandForm } from './brand-form';
import { Brand } from '@atomecom/shared';
import { StudioOverlay } from '@/components/dashboard/studio/studio-overlay';
import { Award } from 'lucide-react';

interface BrandFormOverlayProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: any) => void;
  initialData?: Brand;
  isLoading?: boolean;
}

export function BrandFormOverlay({
  isOpen,
  onClose,
  onSubmit,
  initialData,
  isLoading,
}: BrandFormOverlayProps) {
  const leftContent = (
    <div className="flex flex-col items-center justify-center p-12 text-center">
      <div className="h-20 w-20 rounded-[var(--radius)] bg-foreground/5 flex items-center justify-center mb-4">
        <Award className="h-10 w-10 text-foreground/20" />
      </div>
      <h3 className="text-sm font-black uppercase tracking-widest mb-1">Nhãn hiệu đối tác</h3>
      <p className="text-[10px] text-muted-foreground uppercase font-bold opacity-40">Brand Identity Protocol</p>
    </div>
  );

  const rightContent = (
    <div className="flex-1 overflow-y-auto">
      <BrandForm
        initialData={initialData}
        onSubmit={onSubmit}
        isLoading={isLoading}
      />
    </div>
  );

  return (
    <StudioOverlay
      isOpen={isOpen}
      onClose={onClose}
      leftContent={leftContent}
      rightContent={rightContent}
      maxWidth="max-w-5xl"
    />
  );
}





