        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-6 px-4">
          ...
          <div className="flex items-center gap-4 flex-1 min-w-0">
            {(localSearchTerm ||
              ...
              <Button
                variant="ghost"
                size="icon"
                className="h-11 w-11 rounded-sm hover:bg-muted/10 border-[0.5px] border-border/20 shrink-0 shadow-sm"
                onClick={() => {
                  if (localSearchTerm) {
                    setLocalSearchTerm('');
            ...
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1">
              <Button
                ...
                className={cn(
                  'font-semibold text-lg tracking-tight rounded-md px-4 h-10 transition-all shrink-0',
                  !currentParent
                    ? 'text-foreground'
              ...
              {breadcrumbs.filter(Boolean).map((crumb) => (
                <React.Fragment key={crumb.path}>
                  ...
                  <Button
                    ...
                    className={cn(
                      'text-[10px] font-bold uppercase tracking-wider rounded-md px-4 h-9 transition-all shrink-0',
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            ...
            {onAddAction && (
              <Button
                onClick={onAddAction}
                className="h-11 px-6 rounded-md bg-foreground text-background hover:bg-foreground/90 font-bold text-[10px] uppercase tracking-wide gap-2 shrink-0 shadow-none border border-foreground/10"
              >
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="h-60 rounded-sm border-[0.5px] border-border/20 bg-muted/5 animate-pulse"
              />
            ))}
          </div>
        ) : categories.length === 0 ? (
