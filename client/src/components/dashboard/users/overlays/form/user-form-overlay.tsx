'use client';

import React from 'react';
import { User as UserIcon } from 'lucide-react';
import { User, CreateUserSchema } from '@atomecom/shared';
import { UserForm } from './user-form';
import { StudioOverlay } from '@/components/dashboard/studio/studio-overlay';

interface UserFormOverlayProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CreateUserSchema) => void;
  initialData?: User;
  isLoading?: boolean;
}

export function UserFormOverlay({
  isOpen,
  onClose,
  onSubmit,
  initialData,
  isLoading,
}: UserFormOverlayProps) {
  const leftContent = (
    <div className="flex flex-col items-center justify-center p-12 text-center">
      <div className="h-20 w-20 rounded-[var(--radius)] bg-foreground/5 flex items-center justify-center mb-4">
        <UserIcon className="h-10 w-10 text-foreground/20" />
      </div>
      <h3 className="text-sm font-black uppercase tracking-widest mb-1">Hồ sơ định danh</h3>
      <p className="text-[10px] text-muted-foreground uppercase font-bold opacity-40">User Profile Protocol</p>
    </div>
  );

  const rightContent = (
    <div className="flex-1 overflow-y-auto">
      <UserForm
        user={initialData}
        onSubmit={onSubmit}
        isLoading={isLoading}
      />
    </div>
  );

  return (
    <StudioOverlay
      isOpen={isOpen}
      onClose={onClose}
      leftContent={leftContent}
      rightContent={rightContent}
      maxWidth="max-w-5xl"
    />
  );
}





